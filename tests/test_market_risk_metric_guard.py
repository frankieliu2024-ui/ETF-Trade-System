...
class TestMarketRiskMetricGuard(unittest.TestCase):
    ...
    def test_dashboard_is_fallback_when_no_formal_review_exists(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root / "data" / "state").mkdir(parents=True)
            (root / "data" / "state" / "account_fact.json").write_text(
            ...
            result = build_etf_strategy_risk_metrics(root)
            self.assertEqual(result["etf_strategy_risk_pct"], -6.18)
            self.assertEqual(result["risk_source"], "ETF当前状态_DASHBOARD.md")
            self.assertIn("FORMAL_DASHBOARD_PRIMARY", result["equity_data_quality"])
