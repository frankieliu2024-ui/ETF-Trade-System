...
class NotificationDecisionIdentityTests(unittest.TestCase):
    ...
    def _run(self, item, matches):
        import scripts.notification_center as center
        with tempfile.TemporaryDirectory() as td:
            state = Path(td)
            (state / "execution_reconciliation.json").write_text(
                json.dumps({"status": "RECONCILED", "matches": matches}), encoding="utf-8")
            with patch.object(center, "STATE", state), patch.object(center, "now", return_value=self.NOW):
                return center.revalidate_pending_notifications([item])[0]

    def test_same_code_newer_decision_stays_pending(self):
        item = {"notification_id": "old", "event_type": "PENDING_EXECUTION_CONFIRMATION",
                "related_decision_id": "old", "security_code": "518880",
                "lifecycle_status": "WAITING_CONFIRMATION"}
        matches = [{"intent": {"decision_id": "new", "code": "518880"},
    ...
    def test_exact_reconciled_decision_archives(self):
        item = {"notification_id": "exact", "event_type": "PENDING_EXECUTION_CONFIRMATION",
                "related_decision_id": "d1", "security_code": "159326",
                "lifecycle_status": "WAITING_CONFIRMATION"}
        matches = [{"intent": {"decision_id": "d1", "code": "159326"},
                    "requires_user_confirmation": False}]
        self.assertEqual(self._run(item, matches)["lifecycle_status"], "ARCHIVED")

    def test_exact_confirmation_required_stays_pending(self):
    ...
    def test_ambiguous_legacy_fallback_stays_pending(self):
