...
class Issue656AdoptionShadowReplayTests(unittest.TestCase):
    def _root(self):
        ...
        (root / "config/market/etf_monitor_universe.json").write_text(
            json.dumps({"objects": [{"code": "159781"}]}), encoding="utf-8"
        )
        ...
        (root / "events/trades/historical_trade.json").write_text(
            json.dumps({
                "event_id": "historical_trade",
                "code": "159781",
                "side": "BUY",
                "quantity": 7600,
                "price": 1.305,
                "execution_status": "EXECUTED",
                "executed_at": "2026-07-13T09:36:43+08:00",
                "executed_at_beijing": "2026-07-13T09:36:43+08:00",
                "confirmed_at_beijing": "2026-09-16T06:00:00Z",
                "historical_fact_adopted_at": "2026-09-16T06:00:00Z",
    ...
    def test_polluted_declared_count_contracts_only_by_proven_shadow(self):
        ...
        try:
            ...
            facts = recover_canonical_etf_trade_facts(root, reconstructed, 2)
            self.assertEqual(len(facts), 1)
        finally:
            td.cleanup()
