...
class MarketDivergencePresentationTests(unittest.TestCase):
    ...
    def test_a_share_divergence_uses_formal_names_and_hides_internal_token(self, _labels):
        event = self.event(
            code="A_SHARE_ETF_DIVERGENCE",
            title="【异动提醒】A_SHARE_ETF_DIVERGENCE出现显著分化",
            content="关键结构：领先515880 +4.85%，落后515220 -3.08%，差约7.93个百分点。",
            object_codes=["515880", "515220"],
        )
    ...
    def test_overseas_divergence_keeps_structure_identity_but_hides_token(self, _labels):
        event = self.event(
            code="US_TECH_DIVERGENCE",
            title="【市场异动】US_TECH_DIVERGENCE出现显著分化",
            content="对象/结构：美股科技内部结构；横截面：NDX与SOX。",
