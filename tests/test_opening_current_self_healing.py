...
class OpeningCurrentSelfHealingTests(unittest.TestCase):
    ...
    def test_self_healing_persistence_keeps_canonical_rebuilders_for_repair_actions(self):
        ...
        self.assertGreaterEqual(persist.count("python scripts/build_post_market_review.py"), 2)
        retry_loop = persist.split('for attempt in $(seq 1 "$max_attempts"); do', 1)[1]
        self.assertIn("python scripts/build_post_market_review.py", retry_loop)
