...
class DashboardAccountProjectionTests(unittest.TestCase):
    def setUp(self):
        ...
        p=self.root/"config/market/etf_monitor_universe.json"; p.parent.mkdir(parents=True)
        p.write_text(json.dumps({"objects":[{"code":x} for x in ("561980","588000","159941","159781","159326","518880")]},ensure_ascii=False),encoding="utf-8")
    def tearDown(self): self.temp.cleanup()
    def test_current_schema_is_classified_and_rendered_by_both_producers(self):
        membership=active_account_asset_codes(self.root,self.account)
        self.assertEqual(membership["stocks"],{"300750","601138","301689"})
        self.assertEqual(membership["etf"],{"561980","588000","159941","159781","159326","518880"})
        with patch.object(formal_sync,"ROOT",self.root),patch.object(process_sync,"ROOT",self.root):
            a=formal_sync.build_dashboard_block(self.account,self.equity,"",self.root)
            b=process_sync.build_dashboard_block(self.account,None,{"interaction_scenario":"ACCOUNT_CONFIRMATION"})
        for rendered in (a,b):
            self.assertIn("|宁德时代（300750）|100|393.781|351.000|35,100.00元|-4,278.07元（-10.86%）|",rendered)
            self.assertIn("|黄金ETF（518880）|500|9.118|9.164|4,582.00元|22.95元（+0.50%）|",rendered)
            self.assertIn("持仓ETF：",rendered); self.assertNotIn("持仓ETF：无",rendered)
            self.assertIn("账户个股：",rendered); self.assertNotIn("账户个股：无",rendered)
    def test_legacy_schema_still_works(self):
        legacy=json.loads(json.dumps(self.account))
        for p in legacy["positions"]:
            p["asset_type"]="ETF" if "ETF" in p["name"] else "STOCK"
            p["last_price"]=p.pop("current_price"); p["holding_pnl"]=p.pop("pnl"); p["holding_pnl_pct"]=p.pop("pnl_pct")
        with patch.object(formal_sync,"ROOT",self.root):
            rendered=formal_sync.build_dashboard_block(legacy,self.equity,"",self.root)
