...
券商截图按北京时间自动进入对应场景；真实成交事件优先于时段路由。**用户仅上传券商持仓截图并 `@GitHub`、未附加文字指令时，默认视为请求系统按本节全天自动路由执行对应正式流程，不再询问“要做什么”；截图所在时段决定盘前、集合竞价、盘中、午间或收盘复盘场景。** 默认路由以 `config/runtime_policy.json` 为机器来源：
...
**盘中正式分析强制快速路径**：凡用户提出“盘中分析”“当前判断”“最新行情”“现在行情”，或在 `INTRADAY` 时段仅上传券商持仓截图并 `@GitHub`，以及其他需要当前交易时点事实的正式请求，必须先按**请求发生时的北京时间**重新判定 `interaction_scenario`，不得沿用上一条消息、上一张截图或上一分析节点的时段。若当前为 `INTRADAY`，则在任何大范围GitHub资料扫描之前立即进入查询时补采判断；补采等待期间只读取形成本次判断的最小必要事实：MASTER中相关规则、截图直接解析出的当次账户事实／最新 `account_fact.json`、聚合 `decision_context.json`。**普通无新增成交的盘中截图，其截图事实可直接作为本次判断的最新账户输入，不得为了先完成 `account_fact`、Dashboard或其他文档持久化而阻塞首次正式回复；这些维护在回复后异步完成。若截图显示或用户确认存在真实成交、资金划转或其他会改变持仓/现金/生命周期的账户事件，则仍须先闭环该必要事实。** 除非最小必要事实之间存在冲突或某项具体证据确实可能改变风险许可、机会、金额或卖出动作，否则不得在首次正式回复前串行展开Dashboard全文、经验库、行情档案、`review_context`、`market_delta`、full consistency、维护状态或其他已被 `decision_context` 聚合的重复信息。

**同一用户请求只消费一条查询时刷新链。** 如果请求发生后已经由该请求的账户同步、既有canonical producer或已提交的 `live_snapshot` 请求生成了第一份符合本次场景、质量与时点要求的PASS/可用 `CURRENT`，必须立即消费该快照形成最小充分判断，不得为了获得几分钟后“更更新”的价格再创建第二个即时补采请求。只有尚不存在本次请求后的合格快照时，才创建一个 `requests/live_snapshot/*.json` 查询时补采；创建后不得为同一请求重复创建第二个补采文件。新 `CURRENT` 在 `runtime_policy.json` 的短等待预算内到达即优先使用；补采超时或失败才按现有freshness规则降级回退，并必须明确标注。**在正式回复发送前必须最后再读取一次 `CURRENT`：只要本次请求之后的新PASS/可用CURRENT已经发布，就必须消费该快照并在回复中明确反馈其真实行情时点和足以改变判断的核心行情；不得继续沿用较早的“补采仍在进行”状态，也不得把已经完成的即时补采遗漏在本次正式回复之外。** 首次正式回复只等待会改变当次交易判断的必要输入，其余状态持久化、派生上下文重建、文档维护和一致性审计继续异步完成，不得阻塞用户回复。该规则只约束读取和等待顺序，不新增workflow、checker、provider调用体系或平行decision bundle，不改变MASTER交易规则和生产行情链。

1. 先从截图确认当次账户、持仓、现金和成交事实，并确定 `interaction_scenario`；无新增成交时截图事实直接供本次判断使用，账户正式持久化不阻塞首次回复；有真实成交或其他账户事件时先闭环必要账户事实；
2. 需要当前市场行情时，先检查本次请求后是否已经出现合格canonical PASS `CURRENT`；有则直接消费且停止新增补采，没有才通过一个 `requests/live_snapshot/*.json` 触发查询时补采。查询时行情优先于最近生产快照；午间和收盘后则使用对应已结束A股时段的正式快照，不把请求时间冒充行情时间；
3. 本次请求后的第一份合格核心 `CURRENT` 一旦可用即停止等待并进入正式判断；若唯一一次即时补采尚未完成而最近有效快照仍为FRESH，则按现有短等待/freshness规则回退并明确标注真实时点，不为等待第二次刷新或文档维护延迟正式回复；
...
8. 普通无成交盘中截图先作为本次判断账户事实使用；首次正式回复后再异步更新账户事实和Dashboard云端实时状态区块，不机械改写MASTER、经验库或行情档案；
