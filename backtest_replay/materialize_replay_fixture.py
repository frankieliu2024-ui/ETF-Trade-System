...
def main() -> None:
    ...
    out = root / "replay_sources" / "market_fixture_2026-08-21.json"
    rows = [load_etf(source_dir, code) for code in ETF_CODES]
    rows.extend(load_index(root / "replay_sources", thscode, symbol) for thscode, symbol in INDEX_CODES.items())
