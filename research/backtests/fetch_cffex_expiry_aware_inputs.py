...
YAHOO={"IF":"000300.SS","IC":"000905.SS","IM":"000852.SS"}
...
def fetch_one_spot(product:str,code:str,start:str,end:str)->pd.DataFrame:
    ...
    for fn,arg in ((fetch_spot_eastmoney,code),(fetch_spot_yahoo,YAHOO[product])):
        try:
            ...
            x["product"]=product; print(f"spot {product}: {len(x)} rows via {x['spot_source'].iloc[0]}",flush=True); return x[["trade_date","product","spot_close","spot_source"]]
...
def main():
    ...
    q={"mode":"RESEARCH_ONLY_CFFEX_EXPIRY_AWARE_INPUTS","source":"CFFEX official monthly history archive; spot uses Eastmoney push2his with Yahoo Chart fallback","start":a.start,"end":a.end,"futures_rows":len(f),"spot_rows":len(s),"contracts":{p:int(f[f.product==p].contract.nunique()) for p in PRODUCTS},"date_range":{p:[f[f.product==p].trade_date.min().date().isoformat(),f[f.product==p].trade_date.max().date().isoformat()] for p in PRODUCTS},"spot_sources":{p:sorted(s.loc[s.product.eq(p),"spot_source"].unique().tolist()) for p in PRODUCTS},"expiry_date_source_counts":{k:int(v) for k,v in f.drop_duplicates("contract")["expiry_date_source"].value_counts().to_dict().items()},"expiry_rule":"published third-Friday rule; historical expiries use observed CFFEX trading-day shift when required; contracts expiring after sample end keep nominal rule date without future-data lookahead","production_context_integration":False,"trade_signal":None}
