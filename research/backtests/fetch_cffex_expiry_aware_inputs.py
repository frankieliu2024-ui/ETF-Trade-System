...
def parse_daily_csv(raw:bytes,date:str)->pd.DataFrame:
    ...
    cols=list(x.columns)
    # Stable CFFEX daily-stat positions used by the exchange archive and AKShare parser.
    if len(cols)<11: return pd.DataFrame()
    y=pd.DataFrame({
        "contract":x.iloc[:,0].astype(str).str.strip(),
        "volume":x.iloc[:,4],
        "open_interest":x.iloc[:,6],
        "close":x.iloc[:,8],
        "settlement":x.iloc[:,9],
    })
    y["trade_date"]=pd.Timestamp(date)
    y["product"]=y["contract"].str.extract(r"^([A-Za-z]+)",expand=False).str.upper()
...
def fetch_spot(start:str,end:str)->pd.DataFrame:
    ...
    for p,code in PRODUCTS.items():
        ...
        x=x.dropna(subset=["trade_date","spot_close"]); x["product"]=p; rows.append(x[["trade_date","product","spot_close"]])
