...
**hithink-finance（同花顺金融数据CLI/API）**：主要用于A股ETF、A股指数、当前打新底仓A股行情，以及已验证可覆盖的直接指数。A股ETF和上证／创业板指数为主要生产源；HSTECH优先尝试HS2083直接指数入口。
