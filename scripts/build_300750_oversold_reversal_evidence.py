...
def _fetch_daily_bars() -> list[dict]:
    params = {
        "secid": "0.300750",
        "klt": "101",
        "fqt": "0",
    ...
    req = Request(
        ...
        headers={"User-Agent": "Mozilla/5.0 ETF-Trade-System formal research evidence", "Accept": "application/json"},
    )
...
def build(root: Path = ROOT) -> dict:
    ...
    if len(usable) < 20:
        return {**base, "status": "DEGRADED", "use_in_current_decision": False, "completed_bar_date": cutoff, "coverage": len(usable), "reason": "fewer than 20 completed daily closes"}
    ...
    return {
        **base,
        ...
        "completed_bar_date": latest["date"],
        ...
        "ma20": round(ma20, 4),
        "below_ma20": bool(close_now < ma20),
        "pattern_match": match,
        "direction": "POSITIVE" if match else "INACTIVE",
...
if __name__ == "__main__":
    ...
    print(json.dumps({"status": result.get("status"), "completed_bar_date": result.get("completed_bar_date"), "pattern_match": result.get("pattern_match"), "trade_signal": result.get("trade_signal")}, ensure_ascii=False))
