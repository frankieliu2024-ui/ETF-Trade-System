...
def _decision_market_date(root: Path) -> date:
    ...
    if _is_trading_day(today, calendar):
        return today
    try:
        current_date = date.fromisoformat(str(current.get("market_date") or ""))
        if _is_trading_day(current_date, calendar):
            return current_date
    except ValueError:
        pass
    d = today
...
def _parse_cffex_csv(raw: bytes, trade_date: date) -> list[dict]:
...
def _fetch_day(d: date) -> list[dict]:
    ...
    for scheme in ("https", "http"):
        url = f"{scheme}://www.cffex.com.cn/fzjy/mrhq/{d.year:04d}/{d.month:02d}/{ds}_1.csv"
        try:
            rows = _parse_cffex_csv(_read_url(url, timeout=8), d)
            if rows:
                return rows
            errors.append(f"{scheme}:empty")
        except Exception as exc:
            errors.append(f"{scheme}:{type(exc).__name__}:{exc}")
    raise RuntimeError(f"CFFEX daily {ds} unavailable: {' | '.join(errors)}")
        ...
        try:
            ...
            with zipfile.ZipFile(io.BytesIO(raw)) as archive:
                for name in archive.namelist():
                    ...
                    if not start <= d <= end:
                        continue
                    rows.extend(_parse_cffex_csv(archive.read(name), d))
            if rows:
                return rows
            errors.append(f"{scheme}:empty")
        except Exception as exc:
            errors.append(f"{scheme}:{type(exc).__name__}:{exc}")
    raise RuntimeError(f"CFFEX monthly {ym} unavailable: {' | '.join(errors)}")
...
def _fetch_futures(start: date, end: date, calendar: dict) -> tuple[list[dict], str, list[str]]:
    ...
    while d <= end:
        if _is_trading_day(d, calendar):
            try:
                rows.extend(_fetch_day(d))
            except Exception as exc:
                errors.append(str(exc))
        d += timedelta(days=1)
    unique = {(x["trade_date"], x["product"], x["contract"]): x for x in rows}
    usable_dates = {x["trade_date"] for x in unique.values()}
    if len(usable_dates) >= 8:
        return sorted(unique.values(), key=lambda x: (x["trade_date"], x["product"], x["contract"])), "CFFEX_OFFICIAL_DAILY_CSV", errors

    monthly_rows = []
    monthly_errors = []
    for ym in _month_keys(start, end):
        try:
            monthly_rows.extend(_fetch_month(ym, start, end))
        except Exception as exc:
            monthly_errors.append(str(exc))
    for x in monthly_rows:
        unique[(x["trade_date"], x["product"], x["contract"])] = x
    if not unique:
        raise RuntimeError("; ".join(errors + monthly_errors) or "no CFFEX rows")
    return sorted(unique.values(), key=lambda x: (x["trade_date"], x["product"], x["contract"])), "CFFEX_DAILY_WITH_MONTHLY_FALLBACK", errors + monthly_errors


def _fetch_spot_eastmoney(code: str, start: date, end: date) -> dict[date, float]:
    params = {
        "secid": f"1.{code}", "klt": "101", "fqt": "0",
        "beg": start.strftime("%Y%m%d"), "end": end.strftime("%Y%m%d"),
        "lmt": "100", "fields1": "f1,f2,f3,f4,f5,f6",
        "fields2": "f51,f52,f53,f54,f55,f56,f57,f58,f59,f60,f61",
    }
    url = "https://push2his.eastmoney.com/api/qt/stock/kline/get?" + urlencode(params)
    payload = json.loads(_read_url(url, timeout=10).decode("utf-8"))
    output = {}
    for line in ((payload.get("data") or {}).get("klines") or []):
        ...
        if close is not None and close > 0:
            output[d] = close
    if not output:
        raise RuntimeError(f"eastmoney spot history empty for {code}")
    return output


def _fetch_spot_tencent(code: str, start: date, end: date) -> dict[date, float]:
    sym = "sh" + code
    params = {"param": f"{sym},day,{start.isoformat()},{end.isoformat()},100,qfq"}
    url = "https://web.ifzq.gtimg.cn/appstock/app/fqkline/get?" + urlencode(params)
    payload = json.loads(_read_url(url, timeout=12).decode("utf-8"))
    ...
    for row in rows:
        ...
        if close is not None and close > 0:
            output[d] = close
    if not output:
        raise RuntimeError(f"tencent spot history empty for {code}")
    return output
...
def _select(product: str, mode: str, fact_date: date, rows: list[dict], history: dict[str, list[dict]]) -> dict:
    ...
    return {
        "status": "READY", "selection_mode": mode, "contract": chosen["contract"],
        "expiry_date": (_contract_expiry(chosen["contract"]) or fact_date).isoformat(),
        "trade_date": fact_date.isoformat(), "futures_close": round(point["close"], 4),
        "spot_close": round(point["spot_close"], 4), "basis_close_pct": round(point["basis_close_pct"], 4),
        "basis_change_5d_pct_points": round(delta, 4),
        "direction": "STRENGTHENING" if delta > 0 else ("WEAKENING" if delta < 0 else "FLAT"),
        "open_interest": point["open_interest"], "volume": point["volume"],
    }
...
def build(root: Path = ROOT) -> dict:
    ...
    base = {
        "schema_version": "1.1", "generated_at": datetime.now(timezone.utc).isoformat(timespec="seconds").replace("+00:00", "Z"),
        "mode": "FORMAL_IF_IC_5D_BASIS_DYNAMIC_EVIDENCE", "evidence_id": "if_ic_basis_5d",
        "display_name": "IF／IC 5日基差变化证据", "read_only": True,
        "use_as_decision_evidence": True, "can_generate_decision_independently": False,
        ...
        "point_in_time_rule": "只使用早于decision_market_date的完整逐合约与现货收盘；5日变化先在同一真实合约内计算，再做NEAR/PIT_MAIN选择。",
    }
    ...
    try:
        futures, futures_provider, futures_errors = _fetch_futures(start, cutoff, calendar)
        spots = {}
        spot_providers = {}
        spot_errors = {}
        for product, code in PRODUCTS.items():
            values, provider, errors = _fetch_spot(code, start, cutoff)
            spots[product] = values
            spot_providers[product] = provider
            spot_errors[product] = errors
    except Exception as exc:
    ...
    for product in ("IF", "IC"):
        ...
        products[product] = {
            ...
            "status": "READY" if near.get("status") == "READY" and pit_main.get("status") == "READY" else "DEGRADED",
            "near": near, "pit_main": pit_main, "consensus": consensus,
        }
    ready = all(x.get("status") == "READY" for x in products.values())
    return {
        **base, "status": "READY" if ready else "DEGRADED", "use_in_current_decision": ready,
        "decision_market_date": decision_date.isoformat(), "fact_latest_date": fact_date.isoformat(),
        "provider": {"futures": futures_provider, "spot": spot_providers},
        "provider_diagnostics": {"futures_errors": futures_errors[-8:], "spot_fallback_errors": spot_errors},
        "products": products,
        "interpretation_rule": "IF为主证据、IC为补充证据；正变化只增强、负变化只削弱。NEAR与PIT_MAIN均保留，分歧时显式保留分歧，不生成机械方向或阈值。",
...
if __name__ == "__main__":
    ...
    print(json.dumps({
        "status": result.get("status"), "decision_market_date": result.get("decision_market_date"),
        "fact_latest_date": result.get("fact_latest_date"),
        "IF": ((result.get("products") or {}).get("IF") or {}).get("consensus"),
        "IC": ((result.get("products") or {}).get("IC") or {}).get("consensus"),
        "providers": result.get("provider"), "trade_signal": result.get("trade_signal"),
    }, ensure_ascii=False))
