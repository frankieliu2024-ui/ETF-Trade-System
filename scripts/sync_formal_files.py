...
def normalize_dashboard_projection(text: str, root: Path = ROOT, account: dict | None = None) -> str:
    ...
    # Older runs accidentally persisted the two-character sequence ``\\n``.
    # Normalize that legacy representation before structural processing so the
    # canonical writer repairs the existing document as well as future writes.
