...
def _apac_event() -> dict | None:
    ...
    if 8 * 60 <= minute <= 11 * 60 + 30:
        ...
        return {"key": f"apac-open-signal:{today}:{lead_code}:{direction}", "type": "市场有价值事件", "event_type": "APAC_OPEN_SIGNAL", "title": title, "content": content, "source": "overseas_context+CURRENT", "security_code": lead_code, "security_name": lead_label.split("（")[0], "user_severity": "需要关注", "user_action": "纳入最近A股节点，检查本地传导和直接相关ETF自身反馈，不机械交易", "confirmation_context": {"market": "ASIA", "market_date": today, "direction": direction, "event_category": "APAC_OPEN_SIGNAL", "fact_family": "APAC_OBJECT_DIRECTIONAL_SESSION", "object_codes": [code for code, _, _, _ in selected], "source_fact_ids": [f"{today}:{code}:{(objects.get(code) or {}).get('latest', {}).get('as_of_beijing', '')}" for code, _, _, _ in selected], "session_node": "OPEN_SIGNAL", "lead_code": lead_code, "lead_open_gap_pct": lead_open_gap, "lead_change_pct": lead_current, "tone": tone, "market_as_of_beijing": lead_latest.get("as_of_beijing") or (max(times) if times else ""), "a_share_as_of_beijing": a_time}}
