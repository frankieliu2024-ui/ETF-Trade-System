...
def fetch_daily(code: str, day: str):
    fields = "date,code,close,preclose,tradestatus,pctChg"
...
def main():
    ...
    try:
        ...
        for day in TEST_DATES:
            ...
            chosen = sample_codes([r["code"] for r in normal], SAMPLE_SIZE)
            daily_rows = []
            missing = []
            status_mismatch = []
            ...
            sample_completeness = len(daily_rows) / len(chosen) if chosen else 0.0
            dates.append(
                {
                    "date": day,
                    ...
                    "sample_completeness_ratio": round(sample_completeness, 6),
                    "sample_missing_codes": missing,
                    "sample_trade_status_mismatch_codes": status_mismatch,
                    "sample_direction_counts": direction_counts,
                }
            )
        ...
        daily_sample_pass = all(d["sample_completeness_ratio"] >= 0.98 for d in dates)
        trade_status_pass = all(not d["sample_trade_status_mismatch_codes"] for d in dates)
        delisted_metadata_pass = delisted_count > 0
        qualified = all(
            [historical_universe_pass, metadata_pass, daily_sample_pass, trade_status_pass, delisted_metadata_pass]
        )
        ...
        result = {
            ...
            "stock_basic": {
                "row_count": len(basic),
                "delisted_stock_metadata_count": delisted_count,
            },
            "dates": dates,
            "gates": {
                "historical_universe_pass": historical_universe_pass,
                "metadata_pass": metadata_pass,
                "daily_sample_pass": daily_sample_pass,
                "trade_status_pass": trade_status_pass,
                "delisted_metadata_pass": delisted_metadata_pass,
            },
            "status": "QUALIFIED_FOR_FULL_BREADTH_BACKFILL_POC" if qualified else "NOT_QUALIFIED",
            "next_step_if_qualified": (
                "仅在独立研究任务中按历史交易日重建沪深A股有效交易全集并计算上涨/下跌/平盘家数；"
                "先做有限区间完整回补与交易所抽样交叉验证，再决定是否扩大历史区间。"
            ),
            "decision_boundary": (
                "本PoC只验证数据可得性、PIT股票池和样本日K一致性；不证明市场宽度具有预测力，"
                "不进入MASTER、风险许可、Trial/Confirm、金额或买卖动作。"
            ),
        }
