...
def revalidate_pending_notifications(notifications: list[dict]) -> list[dict]:
    ...
    A security code alone is not sufficient to close an execution prompt because
    one code may have multiple decisions, lifecycle transitions, or fills.
    Ambiguous legacy records remain pending for safe human resolution.
    """
    ...
    for item in notifications:
        ...
        if event_type == "PENDING_EXECUTION_CONFIRMATION":
            ...
            related_decision_id = str(
                item.get("related_decision_id") or context.get("decision_id") or ""
            )
            exact = [
                match for match in matches
                if str((match.get("intent") or {}).get("decision_id") or "")
                == related_decision_id
            ] if related_decision_id else []
            if related_decision_id:
                # An explicit identity must resolve to exactly one canonical match.
                if len(exact) == 1 and not bool(exact[0].get("requires_user_confirmation")):
                    reason = (
                        "exact related decision is canonically reconciled; "
                        "confirmation is no longer required"
                    )
            else:
                # Legacy records may use a constrained identity only when unique.
                ...
                date = str(
                    context.get("suggested_execution_date")
                ...
                candidates = [
                    ...
                    and (
                        not lifecycle
                        or str((match.get("intent") or {}).get("lifecycle") or "") == lifecycle
                    )
