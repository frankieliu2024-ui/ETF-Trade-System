...
def build_dashboard_block(account: dict, decision: dict | None, request: dict) -> str:
    ...
    if decision:
        ...
        lines += ["", f"### {title}", "", f"- 风险许可：{decision.get('risk_permission','未提供')}", *(["- 生命周期："] + _managed_lifecycle_lines(decision.get("lifecycle"), account)) if _managed_lifecycle_lines(decision.get("lifecycle"), account) else [f"- 生命周期：{decision.get('lifecycle','未提供')}"], f"- 唯一主候选：{decision.get('main_candidate','无新的主候选。')}", f"- 金额与动作：{decision.get('amount_action','未提供')}", f"- 最大风险或0元主因：{decision.get('decisive_reason','未提供')}", f"- 决策数据时点：{decision.get('data_as_of_beijing','未提供')}"]
    else:
