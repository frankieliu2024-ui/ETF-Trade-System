...
def main() -> None:
    ...
    atomic_json_write(ROOT / "data" / "state" / "research_context.json", {
        **research,
        "read_only": True,
        "decision_boundary": "研究层直接向当前决策提供事实、相对强弱、日内路径、证据变化、数据质量、判断/执行归因证据；不得绕过MASTER生成交易动作。",
        "current_decision_use": "研究证据及其相对上一节点的变化必须参与机会判断、统一资本比较、持仓资本效率和必要的正式输出解释；单一排名、单一相对强弱或研究统计不得机械产生动作。",
