...
def main() -> None:
    ...
    atomic_json_write(ROOT / "data" / "state" / "research_context.json", {
        **research,
        ...
        "master_feedback": "研究层可形成MASTER维护输入，但retrospective contribution/outcome attribution与MASTER-maintenance inventory属于慢路径/on-demand维护，不在每次当前决策context rebuild中重算；只有通过MASTER第8.1正式研究转化机制的高质量专项研究，或多个真实CASE反复暴露的同类问题，才允许正式修改MASTER。",
